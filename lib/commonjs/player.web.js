...
exports.isCustomSubtitle = exports.getSubtitleFormat = exports.WebOmniPlayer = void 0;
var _react = require("@videojs/react");
var _events = require("./events.web");
const getSubtitleFormat = subtitle => {
...
exports.getSubtitleFormat = getSubtitleFormat;
const isCustomSubtitle = subtitle => {
  const format = getSubtitleFormat(subtitle);
  return format === "ass" || format === "pgs";
};
exports.isCustomSubtitle = isCustomSubtitle;
class WebOmniPlayer {
  onPrev = new Set();
  onNext = new Set();
  constructor(store) {
  ...
  get castStatus() {
    return _events.stateMapper.castStatus.mapper(this._store.state);
  }
  toggleCastStatus() {
  ...
  get status() {
    ...
    return _events.stateMapper.status.mapper(state);
  }
  get isPlaying() {
  ...
  get isAutoQuality() {
    const quality = (0, _react.selectQuality)(this._store.state);
  ...
  get audios() {
    const audio = (0, _react.selectAudioTrack)(this._store.state);
    if (!audio) return [];
  ...
  selectAudio(audio) {
    const tracks = (0, _react.selectAudioTrack)(this._store.state);
    if (!tracks) return;
  ...
  get subtitles() {
    ...
    const textTracks = (0, _react.selectTextTrack)(this._store.state)?.textTrackList ?? [];
  ...
  selectSubtitle(subtitle) {
    ...
    const tracks = (0, _react.selectTextTrack)(this._store.state);
  ...
  selectRendition(rendition) {
    const tracks = (0, _react.selectQuality)(this._store.state);
    if (!tracks) return;
...
exports.WebOmniPlayer = WebOmniPlayer;
